...
class InferenceConfig:
    ...
    inference_dir: Optional[Path] = Path('./save/alibaba/inference_final')
    # Specific path to the mapper you want to load, overrides `input_dir`
    mapper_checkpoint_path: Optional[Path] = Path('./save/alibaba/mapper-steps-3000.pt')
    # Specific path to the embeddings you want to load, overrides `input_dir`
    learned_embeds_path: Optional[Path] = Path('./save/alibaba/learned_embeds-steps-3000.bin')
    ...
    def _set_input_paths(self):
        if self.inference_dir is None:
            assert self.inference_dir is not None, "You must pass an input_dir if you do not specify inference_dir"
            self.inference_dir = self.input_dir / f"inference_final"
        if self.mapper_checkpoint_path is None:
            assert self.mapper_checkpoint_path is not None, "You must pass an input_dir if you do not specify mapper_checkpoint_path"
            self.mapper_checkpoint_path = self.input_dir / f"mapper-steps-final.pt"
        if self.learned_embeds_path is None:
            assert self.learned_embeds_path is not None, "You must pass an input_dir if you do not specify learned_embeds_path"
            self.learned_embeds_path = self.input_dir / f"learned_embeds-steps-final.bin"
...
def main(infer_cfg: InferenceConfig):
    train_cfg, mapper = CheckpointHandler.load_mapper(infer_cfg.mapper_checkpoint_path, get_char_graph())
