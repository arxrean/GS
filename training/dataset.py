...
class TextualInversionDataset(Dataset):
    ...
    def __getitem__(self, i: int) -> Dict[str, Any]:
        path = str(self.image_paths[i % self.num_images])
        ...
        example = dict()
        example['path'] = path
        example['text'] = random.choice(self.templates).format(self.char2idx[path.split('/')[-2]])
        # example['text'] = random.choice(self.templates).format('S0')
