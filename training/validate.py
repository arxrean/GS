...
class ValidationHandler:
    ...
    def __init__(self, cfg: RunConfig, placeholder_token_id: int, weights_dtype: torch.dtype, char2idx: None):
        self.cfg = cfg
    ...
    def infer_on_prompt(self, pipeline: StableDiffusionPipeline,
        ...
        prompt_embeds = self.compute_embeddings(prompt_manager=prompt_manager, prompt=prompt, char2idx=self.char2idx)
        all_images = []
    ...
    @staticmethod
    def compute_embeddings(prompt_manager: PromptManager, prompt: str, char2idx: None) -> torch.Tensor:
        with torch.autocast("cuda"):
            with torch.no_grad():
                prompt_embeds = prompt_manager.embed_prompt(prompt, char2idx)
