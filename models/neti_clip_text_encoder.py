...
class NeTICLIPTextTransformer(CLIPTextTransformer):
    ...
    def forward(self, input_ids: Optional[torch.Tensor] = None,
        ...
        if bypass_output is not None:
            placeholder_idx = torch.where(batch.input_ids>49407)
            # learnable_idxs = (batch.input_ids == batch.placeholder_token_id).nonzero(as_tuple=True)[1]
            existing_state = last_hidden_state_with_bypass[placeholder_idx]
            ...
            last_hidden_state_with_bypass[placeholder_idx] = new_state
